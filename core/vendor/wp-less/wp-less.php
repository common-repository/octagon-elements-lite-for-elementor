...
if ( ! class_exists( 'wp_less' ) ) {
	...
	class wp_less {
		...
		public function __construct() {
			...
			add_filter( 'style_loader_src', [ $this, 'parse_stylesheet' ], 100000, 2 );

			// editor stylesheet URLs are concatenated and run through this filter
			add_filter( 'mce_css', [ $this, 'parse_editor_stylesheets' ], 100000 );
		...
		public function parse_stylesheet( $src, $handle ) {
			...
			$less_version           = md5( serialize( [ $this->vars, $src ] ) );
		...
		public function get_cached_file_data( $path ) {
			$caches = get_option( 'wp_less_cached_files', [] );

...
(function($){
	...
	$( document ).ready( function(){
		...
		$( 'form' ).on( 'click', '.toggle-switch:not(.disable) span', function() {
		...
		$( '.select2' ).each(function( index, el ) {
			...
			var $el = $(el),
				type = $el.data( 'type' ),
				multiple = $el.data( 'allow-multiple' ),
				id = $el.data( 'post-id' ),
				key = $el.data( 'key' ),
			...
			if( 'static' == type ) {
				...
				$el.select2().val( selected ).trigger( 'change' );
		...
		$( '.repeatable-fields' ).on( 'click', '.add-repeatable-field', function( e ) {
			...
			$template.find('.field-group .field').each( function( index, el ) {
				...
				if( 'text' == type || 'color' == type || 'icon_picker' == type ) {
